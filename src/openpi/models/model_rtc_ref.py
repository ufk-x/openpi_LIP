...
class FlowPolicy(nnx.Module):
    ...
    def action(self, rng: jax.Array, obs: jax.Array, num_steps: int) -> jax.Array:
        ...
        noise = jax.random.normal(rng, shape=(obs.shape[0], self.action_chunk_size, self.action_dim)) # shape[b, h, d]
        (x_1, _), _ = jax.lax.scan(step, (noise, 0.0), length=num_steps)
        assert x_1.shape == (obs.shape[0], self.action_chunk_size, self.action_dim), x_1.shape # 验证x_1的形状，如果断言失败，控制台会打印出 x_1.shape 的实际值
        return x_1
    ...
    def realtime_action(
        ...
        def step(carry, _):
            ...
            def pinv_corrected_velocity(obs, x_t, y, t): # y是prev_action_chunk
                def denoiser(x_t):
                    v_t = self(obs[None], x_t[None], t)[0] # self用于获取速度场
                    return x_t + v_t * (1 - t), v_t

                x_1, vjp_fun, v_t = jax.vjp(denoiser, x_t, has_aux=True) # 计算得到hat_x_1和v_t,以及denoiser的vjp函数表示（方便后续求偏导）
                weights = get_prefix_weights(
                    inference_delay, prefix_attention_horizon, self.action_chunk_size, prefix_attention_schedule
                )
                error = (y - x_1) * weights[:, None] # shape [h, d]，将每个时间步的权重广播到动作维度上
                 # 计算vjp，得到伪逆校正项
                pinv_correction = vjp_fun(error)[0] # vjp_fun就是输入一个和denoiser的输出v_t同形状的矢量，返回：矢量的转置 * 函数的雅可比矩阵
